/****************************************************************************
** Meta object code from reading C++ file 'monitor.h'
**
** Created: Sun Oct 16 16:26:25 2011
**      by: The Qt Meta Object Compiler version 62 (Qt 4.7.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "monitor.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'monitor.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.7.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_Monitor[] = {

 // content:
       5,       // revision
       0,       // classname
       0,    0, // classinfo
       8,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: signature, parameters, type, tag, flags
      15,    9,    8,    8, 0x05,

 // slots: signature, parameters, type, tag, flags
      33,    8,    8,    8, 0x0a,
      52,    9,    8,    8, 0x08,
      75,   72,    8,    8, 0x08,
     102,    8,    8,    8, 0x08,
     124,    8,    8,    8, 0x08,
     145,    8,    8,    8, 0x08,
     168,    8,    8,    8, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_Monitor[] = {
    "Monitor\0\0event\0logEvent(QString)\0"
    "onReloadSettings()\0onLogEvent(QString)\0"
    "id\0onRegistratorReceived(int)\0"
    "onRegistratorFailed()\0onChronosAvailible()\0"
    "onChronosUnAvailible()\0onDeviceMoved()\0"
};

const QMetaObject Monitor::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_Monitor,
      qt_meta_data_Monitor, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &Monitor::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *Monitor::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *Monitor::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_Monitor))
        return static_cast<void*>(const_cast< Monitor*>(this));
    return QObject::qt_metacast(_clname);
}

int Monitor::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: logEvent((*reinterpret_cast< QString(*)>(_a[1]))); break;
        case 1: onReloadSettings(); break;
        case 2: onLogEvent((*reinterpret_cast< QString(*)>(_a[1]))); break;
        case 3: onRegistratorReceived((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 4: onRegistratorFailed(); break;
        case 5: onChronosAvailible(); break;
        case 6: onChronosUnAvailible(); break;
        case 7: onDeviceMoved(); break;
        default: ;
        }
        _id -= 8;
    }
    return _id;
}

// SIGNAL 0
void Monitor::logEvent(QString _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}
QT_END_MOC_NAMESPACE
